...
for(let index = 0; index < finances.length; index++){
  for(let index2 = 0; index2 < finances[index].length; index2++) {
     ...
     if(typeof finances[index][index2] !== 'string'){
        ...
        if(change > greastest[1]){
            greastest = [finances[index][0], finances[index][1]]
        }
...
for(let index = 0; index < netArray.length; index++){
    netChallangeSum += netArray[index];
}

average = Math.round((netChallangeSum / finances.length * 100) * 100)
